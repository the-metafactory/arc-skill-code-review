...
**Activated when:** the target repository is **exposed** — public, or arc-shipped (its committed tree ships verbatim to downstream installers). Confidentiality is **exposure-gated, not content-gated**: exposure is a property of the *repo*, not of the diff, so this lens runs on **every** review of an exposed repo regardless of which files changed or which flavor was requested. Always applied in FullReview and SecurityReview; always active in StandardReview when the repo is exposed. It is never skipped because "the diff didn't look sensitive" — a one-line change to an exposed repo can leak.
